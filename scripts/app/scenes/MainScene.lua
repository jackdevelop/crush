...
function MainScene:ctor()
    ...
    local bg = display.newSprite("background.png"):pos(display.cx,display.cy):addTo(self)
    ...
    self.m_levelScore = 0
    self.m_size = {
    	display.width / self.m_colLenght,
    	(display.height - 100) / self.m_rowLenght,
    };
...
--    self.m_matrixLeftBottomY = (display.height - SpriteItem.getContentWidth() * self.m_rowLenght - (self.m_rowLenght - 1) * ITEM_GAP) / 2
	...
	self.m_allLlabelActiveScore = ui.newTTFLabel({text="得分：0", font="", size=24,font = "Arial",color = display.COLOR_RED,align = ui.TEXT_ALIGN_LEFT,dimensions = CCSize(200, 200)})
--    self.m_labelActiveScore:setColor(display.COLOR_RED)
    self.m_allLlabelActiveScore:setPosition(ccp(display.left + 150, display.top - 110))
    self:addChild(self.m_allLlabelActiveScore)
    
    
    self.m_allLlabeltime = ui.newTTFLabel({text="用时：0", font="", size=24,font = "Arial",color = display.COLOR_RED,align = ui.TEXT_ALIGN_LEFT,dimensions = CCSize(200, 200)})
    self.m_allLlabeltime:setPosition(ccp(display.cx + 150, display.top - 110))
    self:addChild(self.m_allLlabeltime)
	self.time_ = 0;

    --每一次销毁 active score lable
    self.m_labelActiveScore = ui.newTTFLabel({text="", font="", size=64,font = "Arial",color = display.COLOR_RED})
--    self.m_labelActiveScore:setColor(display.COLOR_RED)
    self.m_labelActiveScore:setPosition(ccp(display.cx, display.cy + 100))
...
function MainScene:createAndDropItem(row, col, imgIndex)
    ...
    newItem:setContentSize(cc.size(self.m_size[1],self.m_size[2]))
...
function MainScene:positionOfItem(row, col)
    ...
    local x =  self.m_size[1] * (col-0.5)
    local y = self.m_size[2] * (row + 0.5)
...
function MainScene:scorePopupEffect(score, pos)
    ...
    local labelScore = ui.newTTFLabel({text=tmpStr, font="", size=20,color = display.COLOR_RED})
...
--            self:explosiveEffect(temp:getPosition())
    ...
    echoj(self.m_levelScore);
    self.m_allLlabelActiveScore:setString("得分："..self.m_levelScore);
...
--        Timer:start(function() self:passEffectCallback() end, 0.03)
        self:performWithDelay(function() self:passEffectCallback() end, 0.03)
    end
...
function MainScene:tick(dt)	
	...
	if dtt > 1 then
		...
		self.m_allLlabeltime:setString("用时："..GameUtil.formatTime(self.time_));
	end
end
